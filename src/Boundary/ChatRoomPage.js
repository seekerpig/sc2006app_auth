...
import {
  ...
  retrieveMessages,
  createMessage,
} from "../Control/ChatRoomController";
...
function ChatRoomPage() {
  ...
  return (
    <Box className="ChatRoomPage" styles={{ marginTop: "50px" }}>
      ...
      <header>
        <Typography sx={{ align: "center", flexGrow: 1 }}>
          Game Room : {gameId}
        </Typography>
      </header>
      ...
      <section>
        ...
        gameId &&
        currentUserWithProfile.gameList.includes(gameId) ? (
          <ChatRoom user1={currentUserWithProfile} gameId={gameId} />
        ) : (
          <p>Please Sign In With The Correct Account</p>
        )}
      </section>
    </Box>
  );
        ...
        {messages &&
          messages.map(function (msg, i) {
            if (msg.gameID === props.gameId) {
              return (
                <ChatMessage key={i} message={msg} currentUser={props.user1} />
              );
            } else {
              return "";
            }
